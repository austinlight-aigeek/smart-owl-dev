...
def get_user(
    ...
    if not user:
        raise HTTPException(
            detail=f"user with ID {id} does not exist.", status_code=status.HTTP_404_NOT_FOUND
        )
    return user
...
def update_user(
    ...
    if not is_super_user(current_user):
        raise HTTPException(detail="Not a super user!", status_code=status.HTTP_403_FORBIDDEN)

    user = update_user_based_on_email(email=email, user_update=user_update, db=db)
    if not user:
        raise HTTPException(detail=f"user with email '{email}' does not exist.", status_code=404)
    return user
